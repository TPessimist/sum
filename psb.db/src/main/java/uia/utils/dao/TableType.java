...
public class TableType {
  ...
  public CompareResult sameAs(TableType table) {
    ...
    try {
      Map<String, ColumnType> source = this.columns.stream()
          .collect(Collectors.toMap(c -> c.getColumnName(), c -> c));
      Map<String, ColumnType> target = table.columns.stream()
          .collect(Collectors.toMap(c -> c.getColumnName(), c -> c));
  ...
  public CompareResult sameAsSimple(TableType table) {
    CompareResult cr = new CompareResult(this.tableName);
    if (table == null || !this.tableName.equals(table.getTableName())) {
      cr.setPassed(false);
      cr.addMessage("tableName mismatch");
