...
public class ColumnType {
  ...
  public boolean sameAs(ColumnType ct, CompareResult cr) {
    ...
    // STRING
    if (isStringType() && ct.isStringType()) {
      return true;
    }
    // TIMESTAMP
    ...
    if (this.dataType != ct.dataType) {
      cr.setPassed(false);
      cr.addMessage(this.columnName + " dataType not the same: " + this.dataTypeName + " vs "
          + ct.dataTypeName);
      return false;
    }
  ...
  public boolean sameAsSimple(ColumnType ct, CompareResult cr) {
    if (ct == null) {
      cr.setPassed(false);
      cr.addMessage(this.columnName + " not found");
      return false;
  ...
  /**
   * 3. NUMBER 12. VARCHAR 93. TIMESTAMP -101. TIMESTAMP WITH TIME ZONE
   *
   * @param dataType
   */
  ...
  public String getPsSet(int index) {
    ...
    switch (this.dataType) {
      case -101:
      case 93:
        return String.format("DateUtils.setDateTz(ps, %s, data.get%s());",
            index,
  ...
  public String getPsSetEx(int index) {
    ...
    switch (this.dataType) {
      case -101:
      case 93:
        return String.format("DateUtils.setDateTz(ps, %s, %s);",
            index,
            this.propertyName);
      case 3:
  ...
  public String getRsGet(String index) {
    ...
    if ("Timestamp".equals(type)) {
      rsGet = String.format("data.set%s(DateUtils.getDateTz(rs, %s));",
          this.propertyName.substring(0, 1).toUpperCase() + this.propertyName.substring(1), index);
    } else {
      rsGet = String.format("data.set%s(rs.get%s(%s));",
          this.propertyName.substring(0, 1).toUpperCase() + this.propertyName.substring(1), type,
          index);
    }
  ...
  public String toString() {
    return String.format("%-30s, pk:%-5s, %2s, %-15s [%s]", this.columnName, this.pk, this.dataType,
        this.dataTypeName, this.columnSize);
  }

  private boolean isStringType() {
    return this.dataTypeName.contains("VARCHAR") || this.dataTypeName.contains("NCLOB");
  }
}
