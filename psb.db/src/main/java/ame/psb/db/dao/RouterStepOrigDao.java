...
import ame.me.db.RouterStepOrig;
...
public class RouterStepOrigDao {
  ...
  public RouterStepOrigDao(Connection conn) {
    this.conn = conn;
  }

  public int insert(RouterStepOrig data) throws SQLException {
  ...
  public int insert(List<RouterStepOrig> dataList) throws SQLException {
    try (PreparedStatement ps = this.conn.prepareStatement(SQL_INS)) {
      for (RouterStepOrig data : dataList) {
        ps.setString(1, data.getHandle());
        ps.setString(2, data.getRouterBo());
        ps.setString(3, data.getStepId());
        ps.setString(4, data.getDescription());
  ...
  public int update(RouterStepOrig data) throws SQLException {
  ...
  public int update(List<RouterStepOrig> dataList) throws SQLException {
    try (PreparedStatement ps = this.conn.prepareStatement(SQL_UPD)) {
      for (RouterStepOrig data : dataList) {
        ps.setString(1, data.getRouterBo());
        ps.setString(2, data.getStepId());
  ...
  public List<RouterStepOrig> selectAll() throws SQLException {
    ArrayList<RouterStepOrig> result = new ArrayList<RouterStepOrig>();
  ...
  public List<RouterStepOrig> selectByRouterBo(String routerBo) throws SQLException {
    ArrayList<RouterStepOrig> result = new ArrayList<RouterStepOrig>();
  ...
  public RouterStepOrig selectByPK(String handle) throws SQLException {
    RouterStepOrig result = null;
  ...
  private RouterStepOrig convert(ResultSet rs) throws SQLException {
    RouterStepOrig data = new RouterStepOrig();
