...
import ame.psb.db.AlternateComponent;
...
public class AlternateComponentDao {
  ...
  public List<AlternateComponent> selectByProduct(String handle)
      throws SQLException { //通过产品查所有替代料信息
    ArrayList<AlternateComponent> result = new ArrayList<AlternateComponent>();
    try (PreparedStatement ps = this.conn
        .prepareStatement(SQL_SEL + "WHERE handle like ? AND substitute='true' ")) {
      ps.setString(1, "%" + handle + "%");
  ...
  public List<AlternateComponent> selectByBomComponentBo(String bomComponentBo)
    ...
    ArrayList<AlternateComponent> result = new ArrayList<AlternateComponent>();
    try (PreparedStatement ps = this.conn
        .prepareStatement(SQL_SEL + "WHERE BOM_COMPONENT_BO = ? AND substitute='true' ")) {
  ...
  public List<AlternateComponent> selectByBom(String bomComponentBo)
      throws SQLException { //通过产品查所有替代料信息
    ArrayList<AlternateComponent> result = new ArrayList<AlternateComponent>();
    try (PreparedStatement ps = this.conn
        .prepareStatement(SQL_SEL + "WHERE BOM_COMPONENT_BO = ? AND substitute='true' ")) {
