...
public class CdmPoItemDao {
  ...
  public CdmPoItemDao(Connection conn) {
    this.conn = conn;
  }

  public int insert(CdmPoItem data) throws SQLException {
  ...
  public int insert(List<CdmPoItem> dataList) throws SQLException {
    try (PreparedStatement ps = this.conn.prepareStatement(SQL_INS)) {
      for (CdmPoItem data : dataList) {
        ps.setString(1, data.getCustCode());
        ps.setString(2, data.getSapCustCode());
        ps.setString(3, data.getTradCustCode());
        ps.setString(4, data.getPoId());
  ...
  public int update(CdmPoItem data) throws SQLException {
  ...
  public int update(List<CdmPoItem> dataList) throws SQLException {
    try (PreparedStatement ps = this.conn.prepareStatement(SQL_UPD)) {
      for (CdmPoItem data : dataList) {
        ps.setString(1, data.getCustCode());
        ps.setString(2, data.getSapCustCode());
  ...
  public int delete(String waferSn) throws SQLException {
    try (PreparedStatement ps = this.conn
        .prepareStatement("DELETE FROM zm_cdm_po_item WHERE wafer_sn=?")) {
  ...
  public List<CdmPoItem> selectAll() throws SQLException {
    ArrayList<CdmPoItem> result = new ArrayList<CdmPoItem>();
  ...
  public CdmPoItem selectByPK(String waferSn) throws SQLException {
    CdmPoItem result = null;
  ...
  public CdmPoItem selectByWaferid(String waferId) throws SQLException {
    CdmPoItem result = null;
    try (PreparedStatement ps = this.conn.prepareStatement(SQL_SEL + "WHERE lot_wafer_id=?")) {
  ...
  private CdmPoItem convert(ResultSet rs) throws SQLException {
    CdmPoItem data = new CdmPoItem();
